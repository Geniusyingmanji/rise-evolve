...
| Stage 4: Visual-Cognitive Experience Distillation | best-worst 经验蒸馏 | 与 Stage 3 联合，不做离线后处理 | 学到可迁移工具和计划策略 |
| Stage 5: targeted self-evolution | 只对失败族扩数据 | 1K-3K hard cases | 改善 RISE logical/GRADE 弱学科 |

这里的 Stage 3/4 是工程调试顺序，不是两个完全分开的训练范式。正确主线是先用 GRPO-only 跑通 on-policy rollout 和 reward，再在同一批 rollout/update 上加入 `L_GRPO + lambda * L_OPD`。如果 OPD 只用旧轨迹离线蒸馏，就不再是 on-policy distillation，也会重新引入 distribution mismatch。
...
4. **加入 Edit-OPD / Visual-Cognitive Experience Distillation**。这不是 GRPO 结束后的离线第二步，而是 GRPO self-evolution loop 的辅助 loss：同 prompt best/worst rollout 的差异被总结为 experience；teacher branch 看到 region/checklist/diagnostics/experience，student branch 不看；teacher 对同一批 on-policy tokens 重新打分，给 student dense token-level guidance。
...
| Stage 4 | GRPO + Edit-OPD joint objective | 对比 GRPO-only 有额外提升，工具和 region error 下降 |
...
6. 先只开启 experience extraction，人工检查 best/worst summaries 和 retrieval。
7. 接入 teacher context patch、SDL/Edit-OPD loss，在同一 GRPO loop 里扩到 300-800 step。
