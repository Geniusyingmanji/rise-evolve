...
- **Agent 侧 OPD / GenEvolve SDL**：OPD 是同一批 on-policy rollout 上的辅助蒸馏项，目标类似 `L_GRPO + lambda * L_SDL`。它依赖 GRPO rollout 产生的 best/worst pairs 和 diagnostics，不应做成完全离线的 post-hoc distillation。
- **DiffusionOPD / Flow-OPD**：更偏底层 diffusion/flow editor 的 multi-teacher distillation。常见流程是先训练 task-specific teachers，再让 unified student 在自己的 denoising/flow trajectories 上接受 teacher supervision。它适合后续 editor 侧第二阶段，不是 RISEvolve agent 第一阶段的主训练形式。
...
- Edit-OPD 负责把 best-vs-worst 的差异变成 dense token-level decision guidance；它应在 GRPO loop 稳定后作为 joint loss 打开，而不是等 GRPO 训练结束后离线蒸馏。
